...
function BuyerPage() {
  ...
  const fetchParts = async (search = "") => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/spareparts?query=${encodeURIComponent(search)}`
      );
      if (res.ok) {
        const data = await res.json();
        setParts(data);
  ...
  const handleBuyPart = async (part: SparePart) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/spareparts/buy/${part.id}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token || ""}`,
  ...
  // Re-run whenever searchParams in the URL change
  useEffect(() => {
    const typeFromUrl = searchParams.get("type") || "All";
    const conditionFromUrl = searchParams.get("condition") || "All";
  ...
  const handleBuyCar = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/cars/buy/${selectedCar.id}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token") || ""}`,
